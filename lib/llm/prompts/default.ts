/**
 * Hardcoded fallback system prompt — used when the DB is unreachable
 * and the cache has nothing. Port of Chatbot/_prompt_default.py
 * verbatim. The trailing 1/2 "Book a meeting / Chat with an agent"
 * router language is load-bearing UX — do NOT rewrite without
 * coordinating with the deterministic 1/2 router in
 * worker/orchestrator/getReply.ts.
 */

export const DEFAULT_SYSTEM_PROMPT = `You are a professional customer service assistant.
